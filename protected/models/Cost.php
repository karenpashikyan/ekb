...
class Cost extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('request_id, status, user_id', 'numerical', 'integerOnly'=>true),
			array('sum, manager', 'length', 'max'=>250),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, sum, request_id, status, manager, user_id', 'safe', 'on'=>'search'),
		);
	}
